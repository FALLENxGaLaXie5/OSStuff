...
/** global table
 * This table is a list of all files (not directories) that exist within the file system.
 *      Each node has information:
 *          A reference to the descriptor to that file
...
void createFile(int dadIndex)
{
    if (memory[dadIndex].content.fd.size < INDEX_SIZE && memory[dadIndex].content.fd.access >= 0600)
    {
        ...
        //will store the first data location in first spot in index block
        int newFileFirstDataLocation = getFreeBlock_ToggleBit();
        memory[newFileFirstDataLocation].type = DATA;
        memory[newFileIndexLoc].content.index[0] = newFileFirstDataLocation;
...
int main(int argc, char *argv[])
{
    ...
    int processI = openFile(myName,6);
