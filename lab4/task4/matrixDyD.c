...
*/
void allocateAndLoadMatrices(char *fileName, int ***aMatrix, int ***bMatrix, int ***cMatrix, int *m, int *k, int *n)
{
    ...
    *aMatrix = (int **)malloc(*m * sizeof(int *));
    int i;
    for (i = 0; i < *m; ++i)
    {
        *aMatrix[i] = (int *)malloc(*k * sizeof(int));
    }
    *bMatrix = (int **)malloc(*k * sizeof(int *));
    for (i = 0; i < *k; ++i)
    {
        *bMatrix[i] = (int *)malloc(*n * sizeof(int));
    }
    *cMatrix = (int **)malloc(*m * sizeof(int *));
    for (i = 0; i < *m; ++i)
    {
        *cMatrix[i] = (int *)malloc(*n * sizeof(int));
    }
...
/*
pthread_t **multiply(int **aMatrix, int **bMatrix, int **cMatrix, int m, int k, int n)
{
    int rc;
    int r;
    int d;
